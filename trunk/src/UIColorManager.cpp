...
//
//
//	UIColorManager.cpp: implementation of the CUIColorManager class.
//
//
/////////////////////////////////////////////////////////////////////////////
...
CUIPaintManagerColor::CUIPaintManagerColor()
...
CUIPaintManagerColor::CUIPaintManagerColor(COLORREF clr)
...
void CUIPaintManagerColor::Serialize(CArchive& ar)
{
	if (ar.IsStoring())
	{
...
//===========================================================================
// CUIPaintManagerColorGradient class
//===========================================================================

CUIPaintManagerColorGradient::CUIPaintManagerColorGradient()
{
	fGradientFactor = 0.5f;
}
CUIPaintManagerColorGradient::CUIPaintManagerColorGradient(const CUIPaintManagerColor clr)
{
	clrLight = clr;
	clrDark = clr;
	fGradientFactor = 0.5f;
...
CUIPaintManagerColorGradient::CUIPaintManagerColorGradient(const COLORREF clr)
{
	fGradientFactor = 0.5f;
	SetStandardValue(clr);
}

CUIPaintManagerColorGradient::CUIPaintManagerColorGradient(COLORREF clrLight, COLORREF clrDark)
{
	fGradientFactor = 0.5f;
...
void CUIPaintManagerColorGradient::SetStandardValue(const COLORREF clr)
{
	SetStandardValue(clr, clr);
}

void CUIPaintManagerColorGradient::SetStandardValue(COLORREF _clrLight, COLORREF _clrDark, float _fGradientFactor)
{
...
void CUIPaintManagerColorGradient::SetStandardValue(CUIPaintManagerColorGradient& clr)
{
...
void CUIPaintManagerColorGradient::SetCustomValue(const COLORREF clr)
{
	SetCustomValue(clr, clr);
}

void CUIPaintManagerColorGradient::SetCustomValue(CUIPaintManagerColorGradient& clr)
{
...
void CUIPaintManagerColorGradient::SetCustomValue(COLORREF _clrLight, COLORREF _clrDark)
{
	clrLight.SetCustomValue(_clrLight);
	clrDark.SetCustomValue(_clrDark);
}

void CUIPaintManagerColorGradient::Serialize(CArchive& ar)
{
...
CUIColorManager::CUIColorManager()
{
	m_bDisableLunaColors = FALSE;
	m_systemTheme = uiSystemThemeAuto;
	m_winThemeWrapperTheme = uiSystemThemeUnknown;
	m_pfnGetSysColor = 0;
...
CUIColorManager& AFX_CDECL CUIColorManager::Instance()
{
	static CUIColorManager instance;
	return instance;
}


float CUIColorManager::ColorWidth(int nLength, int nWidth)
{
...
float CUIColorManager::ColorDelta(COLORREF clrA, COLORREF clrB)
{
...
float CUIColorManager::Length(COLORREF clrA, COLORREF clrB)
{
	return sqrt(ColorDelta(clrA, clrB));
}

BOOL CUIColorManager::LongColor(COLORREF clrMain, COLORREF clrSub, BOOL bCalcLength, float fDistance)
{
	if (bCalcLength)
...
COLORREF CUIColorManager::MixColor(COLORREF clrMain, COLORREF clrSub, float fDistance)
{
...
COLORREF CUIColorManager::AdjustColor(COLORREF clrMain, COLORREF clrSub, float fDistance)
{
	float Z = sqrt(
...
COLORREF CUIColorManager::LightColor(COLORREF clrLight, COLORREF clrDark, int nDelta) const
{
...
COLORREF CUIColorManager::GetColor(int nIndex) const
{
...
void CUIColorManager::RefreshSysColors()
{
	for (int i = 1; i < 30; i++)
	{
...
void CUIColorManager::SetGetSysColorPtr(PFNGETSYSCOLOR pfnGetSysColor)
{
	m_pfnGetSysColor = pfnGetSysColor;
	RefreshColors();
}

void CUIColorManager::RefreshXPColors()
{
...
void CUIColorManager::RefreshGradientColors()
{
	UICurrentSystemTheme systemTheme = GetCurrentSystemTheme();

	if (m_bDisableLunaColors)
		systemTheme = uiSystemThemeUnknown;

	switch (systemTheme)
	{
	case uiSystemThemeBlue:
	case uiSystemThemeRoyale:
	case uiSystemThemeAero:
		{
...
{
#if 1
	UIColorManager()->RefreshColors(bResetCustomColors);
#else
...
BOOL CUIColorManager::IsLowResolution(HDC hDC/* = 0*/)
{
	if (hDC)
	{
...
void CUIColorManager::ResetCustomColors()
{
...
void CUIColorManager::RefreshColors(BOOL bResetCustomColors /*= TRUE*/)
{
	...
	CUIWinThemeWrapper wrpTreeView;
...
COLORREF CUIColorManager::GetLunaColor(int nIndex) const
{
...
void CUIColorManager::RefreshLunaColors()
{
	UICurrentSystemTheme systemTheme = GetCurrentSystemTheme();
...
void CUIColorManager::SetColors(int cElements, CONST INT* lpaElements, CONST COLORREF* lpaRgbValues)
{
	for (int i = 0; i < cElements; i++)
		m_arrCustomColor[lpaElements[i]] = lpaRgbValues[i];
}

void CUIColorManager::SetColor(int nIndex, COLORREF clrValue)
{
	m_arrCustomColor[nIndex] = clrValue;
}

double CUIColorManager::GetRDelta(COLORREF clr)
{
	return GetRValue(clr) * (195.0/255.0);
}

double CUIColorManager::GetGDelta(COLORREF clr)
{
	return GetGValue(clr) * (390.0/255.0);
}

double CUIColorManager::GetBDelta(COLORREF clr)
{
	return GetBValue(clr) * (65.0/255.0);
}

void CUIColorManager::DisableLunaColors(BOOL bDisable)
{
	m_bDisableLunaColors = bDisable;
}

BOOL CUIColorManager::IsLunaColorsDisabled() const
{
	return m_bDisableLunaColors;
}

void CUIColorManager::SetLunaTheme(UICurrentSystemTheme systemTheme)
{
	m_systemTheme = systemTheme;
	RefreshColors();
}

UICurrentSystemTheme CUIColorManager::GetWinThemeWrapperTheme() const
{
	return m_winThemeWrapperTheme;
}

UICurrentSystemTheme CUIColorManager::_GetWinThemeWrapperTheme()
{
	// windows classic theme is in use we do not need to go further.
	if (!CUIWinThemeWrapper().IsThemeActive())
		return uiSystemThemeUnknown;
	...
	if (CUIWinThemeWrapper().GetCurrentThemeName(pszThemeFileName, MAX_PATH,
...
UICurrentSystemTheme CUIColorManager::GetCurrentSystemTheme() const
{
	if (m_systemTheme != uiSystemThemeAuto && m_systemTheme != uiSystemThemeDefault)
