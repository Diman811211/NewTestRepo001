...
//
//
//	UICustomHeap.h: *** template definition.
//
//
/////////////////////////////////////////////////////////////////////////////

//{{AFX_CODEJOCK_PRIVATE
#if !defined(_UICUSTOMHEAP_H__)
#define _UICUSTOMHEAP_H__
...
//{{AFX_CODEJOCK_PRIVATE
#define UI_EXPORT_PARAMS_NO

typedef enum _UI_HEAP_INFORMATION_CLASS
{
	uiHeapCompatibilityInformation
}
UI_HEAP_INFORMATION_CLASS;
//}}AFX_CODEJOCK_PRIVATE
...
// See Also:
//      HeapSetInformation, UIHeapSetLowFragmentation
//===========================================================================
AFX_INLINE BOOL UIHeapSetCompatibilityInformation(HANDLE hHeapHandle, PVOID pHeapInformation, ULONG_PTR nHeapInformationLength)
{
#ifdef _WIN32_WCE
	return TRUE; // Not supported;
#else
	typedef BOOL (WINAPI *PFNHEAPSETINFORMATION)(HANDLE HeapHandle, UI_HEAP_INFORMATION_CLASS eHICls, PVOID HeapInformation, ULONG_PTR HeapInformationLength);
	...
	if (NULL != hKernel)
	{
		...
		{
			return pfHeapSetInformation(hHeapHandle, uiHeapCompatibilityInformation,
...
//      If HeapSetInformation is not supported by windows return value is also TRUE.
// See Also:
//      HeapSetInformation, UIHeapSetCompatibilityInformation
//===========================================================================
AFX_INLINE BOOL UIHeapSetLowFragmentation(HANDLE hHeapHandle)
{
	// LFH mode does not enabled under debugger,
	// only under 'clear' run.
	ULONG uHI = 2;
	return UIHeapSetCompatibilityInformation(hHeapHandle, &uHI, sizeof(uHI));
}
...
//          };
// See Also:
//      UI_DECLARE_HEAP_ALLOCATOR, UI_IMPLEMENT_HEAP_ALLOCATOR,
//      CUIHeapObjectT
//===========================================================================
template<class _TData>
class CUIHeapAllocatorT : public _TData
{
public:
	//{{AFX_CODEJOCK_PRIVATE
	...
	static void AFX_CDECL CreateHeapIfNeed()
	{
		if (!ms_hCustomHeap)
		{
			...
			ms_bLFHEnabled = UIHeapSetLowFragmentation(ms_hCustomHeap);
		}
	}
...
//{{AFX_CODEJOCK_PRIVATE
#define UI_DECLARE_HEAP_ALLOCATOR_(allocatorClass, EXPORT_PARAMS) \
	...
	class  EXPORT_PARAMS allocatorClass : public CUIHeapAllocatorT<allocatorClass##Data> {};
//}}AFX_CODEJOCK_PRIVATE

//===========================================================================
// Summary:
//      This macros used to declare allocator class derived from CUIHeapAllocatorT
//      which can use default or custom (separate) heap.
//      Such allocator can be used as parameter for CUIHeapObjectT template or
//      for other cases.
// Parameters:
//      allocatorClass  - [in] Name of the allocator class.
// Remarks:
//      Used together with UI_IMPLEMENT_HEAP_ALLOCATOR macro.
// Example:
// <code>
//
//  // probably in header (*.h) file:
//  UI_DECLARE_HEAP_ALLOCATOR(CUIReportRowAllocator)
//
//  // in implementation (*.cpp) file:
//  UI_IMPLEMENT_HEAP_ALLOCATOR(CUIReportRowAllocator)
//
// </code>
// See Also:
//      UI_IMPLEMENT_HEAP_ALLOCATOR, CUIHeapAllocatorT, CUIHeapObjectT
//===========================================================================
#define UI_DECLARE_HEAP_ALLOCATOR(allocatorClass) UI_DECLARE_HEAP_ALLOCATOR_(allocatorClass, UI_EXPORT_PARAMS_NO)


//===========================================================================
// Summary:
//      This macros used to implement allocator class previously declared by
//      UI_DECLARE_HEAP_ALLOCATOR macro.
//      Such allocator can be used as parameter for CUIHeapObjectT template or
//      for other cases.
// Parameters:
//      allocatorClass  - [in] Name of the allocator class.
...
// Remarks:
//      Used together with UI_DECLARE_HEAP_ALLOCATOR macro.
// Example:
// <code>
//
//  // probably in header (*.h) file:
...
// </code>
// See Also:
//      UI_DECLARE_HEAP_ALLOCATOR, CUIHeapAllocatorT, CUIHeapObjectT
//===========================================================================
#define UI_IMPLEMENT_HEAP_ALLOCATOR(allocatorClass, bUseCustomHeap)        \
...
//  // probably in header (*.h) file:
//  UI_DECLARE_HEAP_ALLOCATOR(CMyCustomHeapAllocator)
//
//  // in implementation (*.cpp) file:
//  UI_IMPLEMENT_HEAP_ALLOCATOR(CMyCustomHeapAllocator, TRUE)
//
//  // To enable custom heap allocations use second parameter in
//  // UI_IMPLEMENT_HEAP_ALLOCATOR macro or set corresponding flag
//  // on initialization, before any allocations:
//  //
//  CMyCustomHeapAllocator::ms_bUseCustomHeap = TRUE;
//
//  //*** One way to use:
//  class CMyClass : public CUIHeapObjectT<CMyClassBase, CMyCustomHeapAllocator>
//  {
//      // ...
//  };
...
//  };
//
//  class CMyClass_heap : public CUIHeapObjectT<CMyClass, CMyCustomHeapAllocator>
//  {
//  };
...
class CUIHeapObjectT : public _TObject
{
public:
	//{{AFX_CODEJOCK_PRIVATE
...
// See Also:
//      CUIHeapStringT, UI_DECLARE_HEAP_ALLOCATOR, CUIHeapAllocatorT
//===========================================================================
class CUIHeapString
{
public:
	...
	//      ~CUIHeapString
	//-----------------------------------------------------------------------
	CUIHeapString(LPCTSTR pcszString)
	{
		m_nStrLen = 0;
		m_pcszString = NULL;
...
// See Also:
//      CUIHeapString, UI_DECLARE_HEAP_ALLOCATOR, CUIHeapAllocatorT
//===========================================================================
template<class _TAllocator>
class CUIHeapStringT : public CUIHeapString
{
public:
	//{{AFX_CODEJOCK_PRIVATE
	typedef _TAllocator TAllocator;
	...
	//      ~CUIHeapStringT
	//-----------------------------------------------------------------------
	CUIHeapStringT(LPCTSTR pcszString = _T(""))
	{
		SetString(pcszString);
	}
...
/////////////////////////////////////////////////////////////////////////////
//{{AFX_CODEJOCK_PRIVATE
struct UI_BATCHALLOC_OBJ_HEADER;

struct UI_BATCHALLOC_BLOCK_HEADER
{
	LONG m_dwRefs;
...
#endif
	UI_BATCHALLOC_OBJ_HEADER* pFreeList;

	UI_BATCHALLOC_BLOCK_HEADER* pPrev;
	UI_BATCHALLOC_BLOCK_HEADER* pNext;
};

struct UI_BATCHALLOC_OBJ_HEADER
{
	UI_BATCHALLOC_BLOCK_HEADER* pBlockHeader;

	UI_BATCHALLOC_OBJ_HEADER* pNextFree;

	void* GetData()
	{
		BYTE* pData = (BYTE*)this;
		pData += sizeof(UI_BATCHALLOC_OBJ_HEADER);
		return pData;
	}

	static UI_BATCHALLOC_OBJ_HEADER* GetHeader(void* pData)
	{
		BYTE* pBlockData = (BYTE*)pData;
		pBlockData -= sizeof(UI_BATCHALLOC_OBJ_HEADER);
		return (UI_BATCHALLOC_OBJ_HEADER*)pBlockData;
	}
};

template<class _TAllocator, class _TBatchAllocData>
class CUIBatchAllocManagerT : public _TBatchAllocData
{
public:
	//{{ AFX_CODEJOCK_PRIVATE
	...
	static void* AFX_CDECL AllocData(size_t nSize)
	{
		if (!m_pFreeBlocks)
		{
			int nObjSize = _Round4((int)nSize + sizeof(UI_BATCHALLOC_OBJ_HEADER));
			int nBlockDataSize = sizeof(UI_BATCHALLOC_BLOCK_HEADER) +
			...
			UI_BATCHALLOC_BLOCK_HEADER* pBlockHdr = (UI_BATCHALLOC_BLOCK_HEADER*)pBlockData;
			ZeroMemory(pBlockHdr, sizeof(UI_BATCHALLOC_BLOCK_HEADER));
			...
			UI_BATCHALLOC_OBJ_HEADER* pObjHdr = NULL;
			pBlockData = pBlockData + sizeof(UI_BATCHALLOC_BLOCK_HEADER);
			pBlockHdr->pFreeList = (UI_BATCHALLOC_OBJ_HEADER*)pBlockData;

			for (int i = 0; i < m_nBlockSize; i++)
			{
				pObjHdr = (UI_BATCHALLOC_OBJ_HEADER*)pBlockData;
				//ZeroMemory(pObjHdr, sizeof(UI_BATCHALLOC_OBJ_HEADER));

				pBlockData += nObjSize;

				pObjHdr->pBlockHeader = pBlockHdr;
				pObjHdr->pNextFree = (UI_BATCHALLOC_OBJ_HEADER*)pBlockData;
			}
		...
		UI_BATCHALLOC_OBJ_HEADER* pNextFree = m_pFreeBlocks->pFreeList->pNextFree;
		...
		if (!pNextFree)
		{
			UI_BATCHALLOC_BLOCK_HEADER* pNewBusyBlock = m_pFreeBlocks;
	...
	static void AFX_CDECL FreeData(void* pObj)
	{
		...
		UI_BATCHALLOC_OBJ_HEADER* pObjHdr = UI_BATCHALLOC_OBJ_HEADER::GetHeader(pObj);
		...
		{
			UI_BATCHALLOC_BLOCK_HEADER* pNewFreeBlock = pObjHdr->pBlockHeader;
		...
		{
			UI_BATCHALLOC_BLOCK_HEADER* pEmptyBlock = pObjHdr->pBlockHeader;

			UI_BATCHALLOC_BLOCK_HEADER* pIsNextFree = pEmptyBlock->pPrev ? pEmptyBlock->pPrev : pEmptyBlock->pNext;
...
public:
	...
	{
		UI_BATCHALLOC_BLOCK_HEADER* pBlock = m_pFreeBlocks;
		...
		while (pBlock)
		{
			...
			{
				UI_BATCHALLOC_BLOCK_HEADER* pEmptyBlock = pBlock;
...
//                          protected:
//                              static UI_BATCHALLOC_BLOCK_HEADER* m_pFreeBlocks;  // List of blocks which have free pieces.
//                              static UI_BATCHALLOC_BLOCK_HEADER* m_pBusyBlocks;  // List of blocks which have not free pieces.
//                          };
//
// Example:
// <code>
...
//  // probably in header (*.h) file:
//  UI_DECLARE_BATCH_ALLOC_OBJ_DATA(CBatchReportRecord_Data);
//  class CBatchReportRecord : public CUIBatchAllocObjT<CUIReportRecord, CBatchReportRecord_Data>
//  {
//      // ...
//  };
//
//  // in implementation (*.cpp) file:
//  UI_IMPLEMENT_BATCH_ALLOC_OBJ_DATA(CBatchReportRecord_Data, CBatchReportRecord, TRUE);
//
//  // To enable Batch allocations use second parameter in
//  // UI_IMPLEMENT_BATCH_ALLOC_OBJ_DATA macro or set corresponding flag
//  // on initialization, before any allocations:
//  //
//  CMyCustomHeapAllocator::ms_bUseCustomHeap = TRUE;
//
...
//
// See Also:
//      UI_DECLARE_BATCH_ALLOC_OBJ_DATA, UI_IMPLEMENT_BATCH_ALLOC_OBJ_DATA,
//===========================================================================
template<class _TObject, class _TBatchAllocData, class _TAllocator = _TObject::TAllocator >
class CUIBatchAllocObjT : public _TObject
{
public:
	//{{AFX_CODEJOCK_PRIVATE
	typedef _TObject TObject;
	...
	typedef CUIBatchAllocManagerT<_TAllocator, _TBatchAllocData > TBlockMan;
	//}}AFX_CODEJOCK_PRIVATE
...
//{{AFX_CODEJOCK_PRIVATE
#define UI_DECLARE_BATCH_ALLOC_OBJ_DATA_(dataClass, EXPORT_PARAMS) \
	...
	protected:                                      \
		static UI_BATCHALLOC_BLOCK_HEADER* m_pFreeBlocks;  \
		static UI_BATCHALLOC_BLOCK_HEADER* m_pBusyBlocks;  \
	};
//}}AFX_CODEJOCK_PRIVATE
...
//      CUIBatchAllocObjT template.
// Parameters:
//      dataClass   - [in] Name of the Batch allocation data class.
// Remarks:
//      Used together with UI_IMPLEMENT_BATCH_ALLOC_OBJ_DATA macro.
// Example:
// <code>
//
//  UI_DECLARE_BATCH_ALLOC_OBJ_DATA(CBatchReportRecord_Data);
...
// </code>
// See Also:
//      UI_IMPLEMENT_HEAP_ALLOCATOR, CUIHeapAllocatorT, CUIBatchAllocObjT
//===========================================================================
#define UI_DECLARE_BATCH_ALLOC_OBJ_DATA(dataClass) UI_DECLARE_BATCH_ALLOC_OBJ_DATA_(dataClass, UI_EXPORT_PARAMS_NO)
...
// Remarks:
//      Used together with UI_DECLARE_BATCH_ALLOC_OBJ_DATA macro.
// Example:
// <code>
//
//  UI_DECLARE_BATCH_ALLOC_OBJ_DATA(CBatchReportRecord_Data);
...
// </code>
// See Also:
//      UI_DECLARE_BATCH_ALLOC_OBJ_DATA, CUIHeapAllocatorT, CUIBatchAllocObjT
//===========================================================================
#define UI_IMPLEMENT_BATCH_ALLOC_OBJ_DATA(dataClass, objClass, batchAllocEnabled)  \
	...
	UI_BATCHALLOC_BLOCK_HEADER* dataClass::m_pFreeBlocks       = NULL; \
	UI_BATCHALLOC_BLOCK_HEADER* dataClass::m_pBusyBlocks       = NULL; \
	objClass::TBlockMan gs_##objClass##_BlocksManager;

#endif // !defined(_UICUSTOMHEAP_H__)
